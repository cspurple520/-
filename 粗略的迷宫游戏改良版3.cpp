...
//   func(char front) :   记录当前的方向，判断能否走，判断是否到达终点 
	...
	while(1)
	{
		...
		{
			step++;
			if(a[current_x][current_y-1]==7)
			{
				a[current_x][current_y] = LEFT;
				a[current_x][current_y-1]=6;
				system("cls");
				printmap(); 
				printf("你已到达终点，游戏结束！总步数：%d步\n",step); 
				break;
			}
			if(a[current_x][current_y-1]==1)
			{
				a[current_x][current_y]= LEFT;
				current_y--;
				a[current_x][current_y]=6;
				path[step].x=current_x;
				path[step].y=current_y;
			}
		...
		{
			step++;
			if(a[current_x-1][current_y]==7)
			{
				a[current_x][current_y]=UP;
				a[current_x-1][current_y]=6;
				system("cls");
				printmap(); 
				printf("你已到达终点，游戏结束！总步数：%d步\n",step);
				break;
			}
			if(a[current_x-1][current_y]==1)
			{
				a[current_x][current_y]=UP;
				current_x--;
				a[current_x][current_y]=6;
